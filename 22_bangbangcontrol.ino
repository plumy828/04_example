...
#define _EMA_ALPHA 0.6      // EMA weight (range: 0 to 1)
                          // Setting _EMA_ALPHA to 0 effectively disables EMA filter.

// Servo adjustment
#define _DUTY_MAX 2228  // Servo angle: D degree
#define _DUTY_NEU 1519  // Servo angle: 0 degree
#define _DUTY_MIN 1094   // Servo angle: E degree
#define _SERVO_ANGLE_DIFF 120 // Replace with |D - E| degree
#define _SERVO_SPEED 1000 // servo speed limit (unit: degree/second)

#define _BANGBANG_RANGE 370
...
void loop() {
  ...
  if(event_dist) {
    ...
    dist_filtered = volt_to_distance(ir_sensor_filtered(90, 0.5)); // Replace n with your desired value
...
// Replace the below line with the equation obtained from nonlinear regression analysis
  return 1052 + (-5.55 * a_value) + (0.0103 * pow(a_value, 2)) + (-6.58E-06 * pow(a_value, 3));
}
