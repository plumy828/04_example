...
void loop() {
  ...
  if (distance < _DIST_MIN) {
    distance = distance_old;    // Set Lower Value
    digitalWrite(PIN_LED, 1);       // LED OFF
  } else if (distance > _DIST_MAX) {
    distance = distance_old;    // Set Higher Value
    digitalWrite(PIN_LED, 1);       // LED OFF
  } else {    // In desired Range
    analogWrite(PIN_LED, float abs(distance - 200) * (255 / 100));       // LED ON
